...
def make_catalog(home, destination):
    ...
    opus = copy.deepcopy(next((m for m in models if m['slug'] == 'gpt-5.5'), models[0]))
    opus.update(slug=MODEL, display_name='Claude Opus 5.5 · Claude Code',
                description='Local Claude Code subscription. Native tools and Claude automatic permissions.',
    ...
    def do_POST(self):
        ...
        try:
            ...
            if data.get('model') != MODEL:
                raise ValueError('Only Opus is allowed. No OpenAI fallback exists in this backend.')
            thread_id = self.headers.get('thread-id', '')
            uuid.UUID(thread_id)
        except (ValueError, TypeError):
            return self.reject(400, 'Invalid Opus task request; no model was called.')
...
class Adapter:
    ...
    def remember(self, thread, model=None):
        ...
        if tid not in self.restore:
            ...
            self.restore[tid] = {'model': current if current != MODEL else self.default_model,
    ...
    def bind(self, result):
        ...
        if result['modelProvider'] == PROVIDER:
            self.native.bind(thread['id'], result['cwd'], readonly=result.get('sandbox', {}).get('type') == 'readOnly')
        return result
    ...
    async def switch(self, tid, model, effort=None):
        ...
        provider = PROVIDER if model == MODEL else self.default_provider
    ...
    async def handle(self, method, params):
        ...
        if method == 'config/read':
            ...
            if self.preferences.get('model') == MODEL:
        ...
        if method == 'model/list':
            ...
            if self.preferences.get('model') == MODEL:
                for entry in result['data']:
                    entry['isDefault'] = entry['model'] == MODEL
            return result
        ...
        if method.startswith('thread/realtime/') and tid:
            ...
            if thread.get('modelProvider') == PROVIDER:
                raise ValueError('Voice uses OpenAI. It is disabled for Opus tasks; switch to a native Codex model to use voice.')
    ...
    async def write_config(self, method, params):
        ...
        for edit in edits:
            if edit.get('keyPath') == 'model_provider' and edit.get('value') == PROVIDER:
                raise ValueError('Select Opus per task; the adapter cannot become the global Codex provider.')
        selected = next((e.get('value') for e in edits if e.get('keyPath') == 'model'), None)
        virtual = selected == MODEL or (selected is None and self.preferences.get('model') == MODEL)
        ...
        if method in ('turn/start', 'thread/settings/update'):
            ...
            if model == MODEL:
                ...
                if params.get('permissions') and params['permissions'] != profile:
                    raise ValueError('Changing a named Codex permissions profile during an Opus task is unsupported. Select the profile before starting the task.')
                self.native.bind(tid, params.get('cwd') or current['cwd'], readonly=(
                    params.get('sandboxPolicy') or current.get('sandbox', {})).get('type') == 'readOnly')
            result = await self.core.call(method, params)
            if model == MODEL:
                if params.get('cwd'):
            ...
            return result
        if method == 'thread/resume' and source['modelProvider'] != (PROVIDER if model == MODEL else self.default_provider):
            await self.switch(tid, model)
        if model == MODEL:
