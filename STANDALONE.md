...
| No model fallback | Rejection tests prevent OpenAI/unknown models, attribution mismatches, unauthorized tokens, and messages to unverified/differently modeled tasks from invoking inference. Live native transcripts recorded the selected Claude models. No API-key client is present. Codex reviewer ("Approve for me") requests reach this provider with the task's Claude model and are answered by Claude; a live replay of a captured reviewer request returned a Claude verdict. |
